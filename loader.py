...
class MoleculeDataset_aug(InMemoryDataset):
    ...
    def process(self):
        ...
        data_smiles_series.to_csv(osp.join(self.processed_dir, 'smiles.csv'),
                                  index=False, header=False)
        data_list = [LineGraph()(d) for d in data_list]
...
if __name__ == "__main__":
    ...
    fb_keys = ['lg_x']
    no_edge_datasets = ['clintox', 'tox21', 'toxcast', 'sider']
    edge_embedding1 = torch.nn.Embedding(6, 30)
    edge_embedding2 = torch.nn.Embedding(3, 30)
    for d in datasets:
        dataset = MoleculeDataset(osp.join(dpath, d), dataset=d)
        print(d, dataset, flush=True)
        print(dataset[0])
        continue
        for dd in dataset:
            if dd.edge_index.size(1) == 0:
                print(dd)
    dataset = MoleculeDataset_aug(osp.join(dpath, 'zinc_standard_agent'),
